...
from keras.layers import Dense, Activation, Dropout, Flatten, RepeatVector
...
def MotionFixModel_lstm_one(batch_size, timestep, data_dim, gpu = True):
    ...
    model.add(ConvLSTM2D(150, batch_input_shape=(batch_size, timestep, data_dim), stateful=True, return_sequences=True))
    
    model.add(Dropout(0.2))

    model.add(ConvLSTM2D(150, stateful=True, return_sequences=True))
...
def MotionFixModel_lstm_two(timestep, data_dim, gpu = True):
    ...
    if gpu:
        model.add(ConvLSTM2D(100, input_shape=(timestep, data_dim), return_sequences=True))
    else:
    ...
    if gpu:
        model.add(ConvLSTM2D(100, return_sequences=True))
    else:
